...
class PartBarcode(models.Model):
    ...
    # 64 rồi 255 đều từng bị vỡ trên thực tế — QR nhiều trường của Tokin có
    # thể dài hơn ước tính (đã gặp value too long ở cả 2 mốc). QR chuẩn chứa
    # được tới ~4000 ký tự alphanumeric, nới hẳn lên 2000 cho chắc, khỏi phải
    # sửa lại lần nữa.
    code       = models.CharField(max_length=2000, unique=True, db_index=True)
